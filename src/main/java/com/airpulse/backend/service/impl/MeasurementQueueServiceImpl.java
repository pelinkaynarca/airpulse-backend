...
import com.airpulse.backend.service.MeasurementQueueService;
...
@Slf4j
public class MeasurementQueueServiceImpl implements MeasurementQueueService {
    ...
    @Override
    public void sendToQueue(AirQualityMeasurementRequest measurement) {
        try {
            log.info("Sending measurement to queue: parameter={}, value={}, lat={}, lon={}",
            ...
            log.info("Successfully sent measurement to queue");
        } catch (Exception e) {
            log.error("Failed to send measurement to queue: {}", e.getMessage(), e);
